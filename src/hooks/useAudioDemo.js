...
/**
 * Custom hook to control live dual-audio comparison (before/after noise cancellation or accent conversion).
 */
export function useAudioDemo(initialTrackId = "remote-work") {
  ...
  // Initialize and load synchronized audio tracks
  useEffect(() => {
  ...
  // Track playback time and progress percentage
  ...
  // Synchronize mute/unmute between raw vs AI-processed audio
  useEffect(() => {
  ...
  // Teardown audio elements on component unmount
  useEffect(() => {
    return () => {
      if (audioBeforeRef.current) {
